...
export async function recalculateWBSProgress(projectId: string): Promise<void> {
  ...
  const updates: Array<{ id: string; progress: number; status: WBSItemStatus }> = [];

  // Recursive post-order traversal to compute parent progresses
  const calculateNode = (nodeId: string): { progress: number; status: WBSItemStatus } => {
    ...
    const computedProgress = totalWeight > 0 ? parseFloat((weightedProgressSum / totalWeight).toFixed(2)) : 0;
    ...
    } else if (childStatuses.some(s => s === 'In Progress' || s === 'Completed' || s === 'Delayed')) {
    ...
    if (node.progress !== computedProgress || node.status !== computedStatus) {
      updates.push({ id: nodeId, progress: computedProgress, status: computedStatus });
  ...
  const overallProjectProgress = totalRootWeight > 0 ? parseFloat((rootWeightedProgressSum / totalRootWeight).toFixed(2)) : 0;
