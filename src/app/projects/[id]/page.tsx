...
import { FolderKanban, Flag, Sliders, Calendar, Loader2, Activity, Target, TrendingUp } from 'lucide-react';
...
export default function ProjectDetailPage() {
        ...
        <div className="bg-gradient-to-r from-slate-600 via-gray-700 to-slate-800 text-white py-12 px-6 shadow-md">
          <div className="container mx-auto max-w-6xl">
            <div className="flex items-center space-x-3 mb-3">
              ...
              <Badge variant="secondary" className="bg-slate-500/30 text-slate-100 hover:bg-slate-500/40 border-slate-400/20">
                Project Details
              </Badge>
            </div>
        ...
        <div className="container mx-auto max-w-5xl py-8 px-6">
          <div className="flex justify-center items-center py-20">
            <Card className="max-w-md text-center">
              <CardHeader>
                <CardTitle className="text-slate-800">Project Not Found</CardTitle>
  ...
  const overallProgress = totalRootWeight > 0 ? Math.round(rootWeightedProgressSum / totalRootWeight) : 0;
        ...
        <div className="container mx-auto max-w-5xl">
          <div className="flex justify-between items-start flex-wrap gap-4">
            <div>
              ...
              <p className="text-blue-100 mt-2 max-w-2xl">{project.description || 'No description provided.'}</p>
            </div>
            <Badge 
              variant={
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            <div className="bg-white/10 backdrop-blur-sm p-4 rounded-lg border border-white/20">
              ...
              <p className="text-lg font-semibold mt-1">{new Date(project.start_date).toLocaleDateString()}</p>
            </div>
            ...
            <div className="bg-white/10 backdrop-blur-sm p-4 rounded-lg border border-white/20">
              ...
              <p className="text-lg font-semibold mt-1">{new Date(project.end_date).toLocaleDateString()}</p>
            </div>
            
            <div className="bg-white/10 backdrop-blur-sm p-4 rounded-lg border border-white/20">
              <div className="flex items-center gap-2 text-blue-100">
                <Target className="h-4 w-4" />
                <span className="text-xs uppercase tracking-wide">Overall Progress</span>
              </div>
              <p className="text-lg font-semibold mt-1 text-center">{overallProgress}%</p>
            ...
            <CardContent>
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-sm font-semibold text-slate-700">{new Date(project.start_date).toLocaleDateString()}</p>
                </div>
                <div>
                  <span className="text-xs text-slate-400 font-medium">End Date</span>
                  <p className="text-sm font-semibold text-slate-700">{new Date(project.end_date).toLocaleDateString()}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardContent className="flex flex-col justify-center">
              <div className="space-y-2">
                ...
                <p className="text-center text-lg font-bold text-indigo-600">{overallProgress}%</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-white border-slate-100 shadow-sm hover:shadow-md transition-shadow">
            ...
            <CardContent>
              <div className="flex flex-col space-y-2">
                <Button variant="outline" size="sm" asChild className="border-indigo-100 text-indigo-700 hover:bg-indigo-50 justify-start">
                  <Link href={`/projects/${project.id}/wbs`} className="flex items-center gap-1.5">
                    <FolderKanban className="h-4 w-4" /> Manage WBS Structure
                  </Link>
                </Button>
                <Button variant="outline" size="sm" asChild className="border-emerald-100 text-emerald-700 hover:bg-emerald-50 justify-start">
                  <Link href={`/projects/${project.id}/milestones`} className="flex items-center gap-1.5">
